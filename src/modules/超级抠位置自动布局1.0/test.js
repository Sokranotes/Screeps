...
let managerplanner={
     ...
     * 优化不动了
     */
    getBlockPutAbleCnt:function (roomWalkable,visited,queMin,unionFind,tarRoot,putAbleCacheMap,AllCacheMap){
        ...
        roomWalkable.forEach((x, y, val)=>{
            if(!roomManor.get(x,y)||roomObjectCache.get(x,y)) {
                ...
                // if(roomObjectCache.get(x,y))visual.text(val&&!roomObjectCache.get(x,y)?-4:-1, x,y+0.25, {color: 'red',opacity:0.99,fontSize: 7})
                ...
                if(!visited.exec(x,y,1)){
                    ...
                    if(roomManor.get(x,y)){
                        if(nd.k+1>=0&&val){
                            ...
                            // visual.text(nd.k+2, x,y+0.25, {color: 'red',opacity:0.99,fontSize: 7})
                        }
                        if(val)
     ...
     * @param roomName
     */
    computeBlock:function (roomName){
        const visual = new RoomVisual(roomName);
        ...
        for(let i=0;i<objects.length;i++){
            ...
            // link 位置
            if(i!=1)getObjectPos(p[0],p[1],"link")
        }
        ...
        queMin.whileNoEmpty(nd=>{
            ...
            posSeqMap[pos].forEach(e=>{
                ...
                roomWalkable.forNear((x,y,val)=>{
                    if(val&&!visited.exec(x,y,1)){
                        ...
                        allNearCnt+=1
                        // if(i==104)
                        // visual.text(parseInt(1*10)/10, x,y+0.25, {color: "cyan",opacity:0.99,fontSize: 7})
            ...
            if(minSize>minPlaneCnt)
                targetBlockPutAbleCnt = managerplanner.getBlockPutAbleCnt(roomWalkable,visited,queMin2,unionFind,targetPos,putAbleCacheMap,allCacheMap)[0].length
            if(nd.k>minPlaneCnt)
                ndkBlockPutAbleCnt = managerplanner.getBlockPutAbleCnt(roomWalkable, visited, queMin2, unionFind, nd.v,putAbleCacheMap,allCacheMap)[0].length
            ...
            //     // visual.text(nd.k+"+"+minSize+"="+cnt, (x+x1)/2,(y+y1)/2-0.25, {color: "red",opacity:0.99,fontSize: 7})
            //     // visual.text(allNearCnt+"_"+nearCntMap[targetPos]+" "+nearCnt+" "+parseInt(nearCnt/Math.sqrt(Math.min(minSize,nd.k))*100)/100+" "+parseInt(maxRatio-Math.sqrt(nd.k)/12*100)/100, (x+x1)/2,(y+y1)/2+0.25, {color: "yellow",opacity:0.99,fontSize: 7})
            //     visual.text(parseInt(targetBlockPutAbleCnt*100)/100+" "+parseInt(ndkBlockPutAbleCnt*100)/100, (x+x1)/2,(y+y1)/2+0.25, {color: "yellow",opacity:0.99,fontSize: 7})
            // }
        ...
        //     let x = ((e-y)/50)//Math.round
        //     let color = "red"
        //     let cnt = managerplanner.getBlockPutAbleCnt(roomWalkable,visited,queMin2,unionFind,e,putAbleCacheMap).length
        //     managerplanner.getBlockPutAbleCnt(roomWalkable,visited,queMin2,unionFind,e,putAbleCacheMap).forEach(t=>{
        //         visual.circle(t.x, t.y, {fill: randomColor(e), radius: 0.5 ,opacity : 0.35})
        //     })
        //     // let cnt = sizeMap[e]
        //     if(sizeMap[e]>0)visual.text(parseInt(cnt*10)/10, x,y+0.25, {color: color,opacity:0.99,fontSize: 7})
        // })
        ...
        // nearWall.forEach((x, y, val)=>{if(val)visual.text(parseInt(val*10)/10, x,y+0.25, {color: "red",opacity:0.5,fontSize: 7})})
     ...
     */
    computeManor:function (roomName,points){
        managerplanner.init()
        for(let p of points){
            if(p.pos&&p.pos.roomName==roomName)objects.push(p.pos)
        }
        // const visual = new RoomVisual(roomName);
        let blockArray = managerplanner.computeBlock(roomName)
        ...
        _.keys(sizeMap).forEach(pos=>{
            ...
            // currentPutAbleList.forEach(e=>visual.text(e.k, e.x,e.y+0.25, {color: 'red',opacity:0.99,fontSize: 7}))
                ...
                roomWalkable.forNear((x,y,val)=>{
                    if(!visited.exec(x,y,1)&&val){
                        if(!roomManor.get(x,y))
                            ...
                            // visual.text('X', x,y+0.25, {color: 'red',opacity:0.99,fontSize: 7})
                        }
                    }
                },nd.x,nd.y)
            })
            ...
            //     visual.text(parseInt((allList.length)*10)/10, x,y, {color: "yellow",opacity:0.99,fontSize: 7})
            //     visual.text(parseInt((currentPutAbleList.length)*10)/10, x,y+0.5, {color: "red",opacity:0.99,fontSize: 7})
            //     visual.text(parseInt((currentInnerPutAbleList.length)*10)/10, x,y+1, {color: "red",opacity:0.99,fontSize: 7})
            // }
        ...
        // innerPutAbleList.forEach(e=>visual.text(e.k, e.x,e.y+0.25, {color: 'red',opacity:0.99,fontSize: 7}))
        ...
        roomManor.forEach((x,y,val)=>{
            if(val>=2){
                // if(roomManor.get(x,y)>0&&Math.abs(x-storageX)+Math.abs(y-storageY)>2)
                    // visual.text(val, x,y+0.25, {color: 'cyan',opacity:0.99,fontSize: 7})
        ...
        // visual.text("C", centerX,centerY+0.25, {color: 'green',opacity:0.99,fontSize: 7})
        // visual.text("S", storageX,storageY+0.25, {color: 'blue',opacity:0.99,fontSize: 7})
        // visual.text("L", labX+0.5,labY+0.75, {color: 'blue',opacity:0.99,fontSize: 7})
        // clog(roomName)
        ...
        //     let x = ((e-y)/50)//Math.round
        //     let color = "red"
        //     if(sizeMap[e]>0)visual.text(parseInt(sizeMap[e]*10)/10, x,y+1+0.25, {color: color,opacity:0.99,fontSize: 7})
        // })
        ...
        // nearWall.forEach((x, y, val)=>{if(val>2&&finalPos==unionFind.find(x*50+y))visual.text(nearWall.get(x,y),x, y+0.5, {color: "red",opacity:0.99,fontSize: 7})})
        ...
        // 资源点布局
        roomObjectCache.forEach((x,y,val)=>{
            if(val){
                structMap[val].push([x,y])
            }
        })
        //中心布局
        ...
        walls.forEach(e=>structMap[((e.x+e.y)%2==0)?"rampart":"constructedWall"].push([e.x,e.y]))
        ...
        queMin.whileNoEmpty(nd=>{
            ...
            // visual.text(nd.k,nd.x,nd.y+0.25, {color: "pink",opacity:0.99,fontSize: 7})
        })
        ...
        roomStructs.forEach((x,y,val)=>{
            if (val > 0) {
                ...
                if(val==12){// 8 && 12 上面有写，注意！！！
                    ...
                    // visual.text(dist,x, y+0.25, {color: "pink",opacity:0.99,fontSize: 7})
                }
            }
        })
        ...
        roomStructs.forEach((x,y,val)=>{
            ...
            // visual.text(val,x, y+0.25, {color: "pink",opacity:0.99,fontSize: 7})
            let minVal =50;
            costRoad.forNear((x1,y1,val)=>{
                if(minVal>val&&val>0){
                    minVal = val
        ...
        queMin.whileNoEmpty(nd=>{
            ...
            costRoad.set(nd.x,nd.y,nd.k)
            // visual.text(costRoad.get(nd.x,nd.y),nd.x,nd.y+0.25, {color: "pink",opacity:0.99,fontSize: 7})
        })
        ...
        structMap["container"].forEach(e=>{
            let dfsBack = function (x,y){
                ...
                // visual.text(minVal,finalX,finalY+0.25, {color: "pink",opacity:0.99,fontSize: 7})
            }
            dfsBack(e[0],e[1])
